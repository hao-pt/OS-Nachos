...
PTable::PTable(int size)
{
    ...
    psize = size; // Kich thuoc cua bang page table
    bm = new BitMap(size); // Cap phat mang bitmap
    bmsem = new Semaphore("bmsem",1);

    For(i,0,MAX_PROCESS){
		pcb[i] = 0; // Gan 0 tuc la NULL cho cac pcb
    }
	
	// Danh dau thang cha da co trong pTable
	bm->Mark(0);
	
	// Khoi tao cho thang cha
	pcb[0] = new PCB(0);
