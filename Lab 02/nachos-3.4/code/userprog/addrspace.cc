...
AddrSpace::AddrSpace(OpenFile *executable)
{
	...
	// Kiem tra neu so khung trang trong khong du de chay chuong trinh moi
...
// --------------------------------- Ham khoi tao AddrSpace chinh sua --------------------------
AddrSpace::AddrSpace(char* filename)
{
    NoffHeader noffH;
    unsigned int i, size;
	
	// Kiem tra su ton tai cua 1 chuong trinh thi dung phuong thuc Open trong filesys
    OpenFile* executable = fileSystem->Open(filename);
	
	// Neu filename khong ton tai
    if (executable == NULL)
    {
    	printf("Unable to open file %s\n", filename);
	    return;
	...
	// Giup chuong trinh duoc doc quyen truy xuat
 	addrLock->P();
