		...
		case SyscallException:{
			switch (type){
				...
				case SC_Exec:
				{
					...
					// Output: Fail return -1, Success: return ProcessID cua chuong trinh nguoi dung vua duoc tao
					// Cong dung: Exec system call gọi thực thi một chương trình mới trong một system thread mới.
					...
					if(mythread == NULL){
						...
						printf("\nSC_Exec: Not enough memory!\n");
						machine->WriteRegister(2,-1); // Tra ve -1 neu that bai
						IncreasePC();
						return;
					}
					
					// b. Load chương trình vào khoảng bộ nhớ mới được cấp phát
					// Tim vi tri con trong trong page table
					pid = pageTable->Find();
					// Check if have free slot
					// Neu page table het cho chua
					if(pid < 0)
					{
						DEBUG('a', "\nSC_Exec: There is no free slot.\n");
						printf("\nSC_Exec: There is no free slot.\n");
						machine->WriteRegister(2,-1); // Tra ve -1 neu that bai
						IncreasePC();
						return;
					}
					// Luu ten chuong trinh vao fileNameTable
					fileNameTable[pid] = name;
					// Parent la processID cua currentThread
				...
				case SC_Wait:
				{
					// Cu phap: void Wait(SpaceId id);
					// Input: ProcessID
					// Cong dung: Chuyen tien trinh hien tai sang trang thai cho
					
					int id = machine->ReadRegister(4); // Đọc exitStatus từ thanh ghi r4
					...
					{
						DEBUG('a', "\nSC_Wait: Can't wait in process that is not its parent process.\n");
						printf("\nSC_Wait: Can't wait in process that is not its parent process.\n");
						IncreasePC();
						return;
					}
